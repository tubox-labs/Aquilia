...
1. `aquilia.aquilia_mcp.context` scans and indexes `aquilia/**/*.py`, `docs/**/*.md`, `examples/**/*`, and `tests/**/test_*.py`.
2. `aquilia.aquilia_mcp.registry` exposes deterministic tools and prompts with strict schemas.
3. `aquilia.aquilia_mcp.server.AquiliaMCPServer` maps MCP JSON-RPC methods to registry calls and resource/prompt access.
4. `aquilia.aquilia_mcp.transport.stdio` provides bounded newline-delimited JSON-RPC over stdin/stdout.
