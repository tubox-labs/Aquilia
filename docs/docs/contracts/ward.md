...
A **Ward** is an explicit cross-field validator registered on an Aquilia [Contract](../../../aquilia/contracts/core.py#L826). As defined in [ward.py](../../../aquilia/contracts/ward.py), wards replace the legacy prefix-based scanning of `seal_*` or `async_seal_*` methods with an explicit decorator-driven registration system discovered once at class-body evaluation time (see [ward.py lines 2–6](../../../aquilia/contracts/ward.py#L2-L6)).
...
#### Under the Hood
* When used as a bare decorator `@ward`, the [ward](../../../aquilia/contracts/ward.py#L57) class intercepts the method call during class evaluation via its `__new__` method (see [ward.py lines 80–90](../../../aquilia/contracts/ward.py#L80-L90)).
* It validates that the decorated object is indeed a callable (raising a `TypeError` if not).
* It registers validation metadata on the function under `fn.__ward_meta__` as `{"mode": "sync", "name": fn.__name__}` (see [ward.py line 88](../../../aquilia/contracts/ward.py#L88)).
...
#### Under the Hood
* Calling `@ward(mode="async")` invokes `__new__` without a decorated function, returning a new instance of the [ward](../../../aquilia/contracts/ward.py#L57) class acting as a decorator factory (see [ward.py lines 91–95](../../../aquilia/contracts/ward.py#L91-L95)).
* The `mode` parameter is validated against the valid modes list, raising a `ValueError` if the specified mode is invalid.
* When the returned `ward` instance is subsequently called with the decorated function (`fn`), its `__call__` method attaches the `__ward_meta__` dictionary using the stored mode (see [ward.py lines 105–110](../../../aquilia/contracts/ward.py#L105-L110)).

---

### The `WardMethod` Dataclass

The metadata for each registered ward is stored in an instance of the [WardMethod](../../../aquilia/contracts/ward.py#L42-L47) class, a frozen, memory-optimized dataclass (using slots) defined in [ward.py lines 42–47](../../../aquilia/contracts/ward.py#L42-L47).
...
The validation engine supports a restricted set of modes defined in the internal constant `_VALID_MODES` (see [ward.py line 54](../../../aquilia/contracts/ward.py#L54)):

* `"sync"`: Standard synchronous validation.
* `"async"`: Asynchronous validation (requires an event loop to resolve).

Any value passed to the decorator's `mode` parameter that is not in `_VALID_MODES` raises a `ValueError` during decoration (see [ward.py lines 81–82](../../../aquilia/contracts/ward.py#L81-L82)).

---

### Method Collection: `collect_ward_methods()`

During Contract class creation, all registered wards are harvested using the [collect_ward_methods](../../../aquilia/contracts/ward.py#L118) helper function (see [ward.py lines 118–199](../../../aquilia/contracts/ward.py#L118-L199)).

#### Inheritance and Override Semantics
1. **Inheritance Scan**: The function iterates through the base classes (`bases`) in Method Resolution Order (MRO) to collect inherited ward methods (see [ward.py lines 147–150](../../../aquilia/contracts/ward.py#L147-L150)).
2. **Namespace Scan**: It then scans the current class namespace for attributes decorated with `@ward` (identifiable by the `__ward_meta__` attribute) (see [ward.py lines 152–161](../../../aquilia/contracts/ward.py#L152-L161)).
3. **Override Mapping**: The inherited and own methods are merged. If a subclass defines a ward with the same name as an inherited ward, the subclass's version replaces (overrides) the parent's version in the merged dictionary (see [ward.py lines 182–185](../../../aquilia/contracts/ward.py#L182-L185)).
...
The list of collected [WardMethod](../../../aquilia/contracts/ward.py#L42-L47) descriptors is stored on the evaluated Contract class as a class attribute named `_ward_methods` (see [ward.py lines 148–150](../../../aquilia/contracts/ward.py#L148-L150)).
...
* **Usage in `ward.py`**: The decorator and module docstring examples call `self.reject(field, message)` to report specific validation errors (e.g., `self.reject("total", "...")` in [ward.py lines 17, 22](../../../aquilia/contracts/ward.py#L17)).
* **Actual Implementation**: "Unknown from inspected source" (the actual `self.reject()` method is not implemented or defined within [ward.py](../../../aquilia/contracts/ward.py), although its usage in validator methods is demonstrated in the module docstring on lines 17 and 22. According to `.cache/index_contracts.json`, `reject()` is a method on the base [Contract](../../../aquilia/contracts/core.py#L826) class defined in `aquilia/contracts/core.py` at lines 1250–1260).
...
1. **Inherited Wards First**: Wards defined in parent base classes are run first, preserving MRO and definition order (see [ward.py lines 139–141, 191–193](../../../aquilia/contracts/ward.py#L139-L141)).
2. **Subclass Wards Second**: Wards defined directly on the subclass are run next in their order of definition (see [ward.py lines 141–142, 194–197](../../../aquilia/contracts/ward.py#L141-L142)).
