...
import {
  ...
  Rocket, Box, Terminal, Code2, GitBranch, Copy, ArrowRight
} from 'lucide-react'
    ...
    email = CharField(max_length=255, unique=True)`}
      ...
      <section className="mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { to: '/docs/installation', icon: <Rocket className="w-4.5 h-4.5" />, title: 'Installation', desc: 'Install Aquilia and set up your environment' },
            { to: '/docs/quickstart', icon: <Zap className="w-4.5 h-4.5" />, title: 'Quick Start', desc: 'Build your first API in 5 minutes' },
            { to: '/docs/architecture', icon: <Cpu className="w-4.5 h-4.5" />, title: 'Architecture', desc: 'Understand the manifest-driven pipeline' },
            { to: '/docs/controllers/overview', icon: <Layers className="w-4.5 h-4.5" />, title: 'Controllers', desc: 'Class-based request handlers with DI' },
            { to: '/docs/di/container', icon: <Plug className="w-4.5 h-4.5" />, title: 'Dependency Injection', desc: 'Six-scope DI container with async support' },
            { to: '/docs/models/defining', icon: <Database className="w-4.5 h-4.5" />, title: 'Models & ORM', desc: 'Metaclass-driven ORM with migrations' },
          ].map((link, i) => (
            <Link
              key={i}
              to={link.to}
