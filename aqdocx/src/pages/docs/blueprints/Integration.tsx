...
export function ContractsIntegration() {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-12">
        <div className="flex items-center gap-2 text-sm text-aquilia-500 font-medium mb-4">
          ...
          Contracts / Controller Integration
        </div>
        ...
        <p className={`text-lg leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Contracts integrate deeply with Aquilia's Controllers, DI Container, Auth system, and Sessions. This page shows how Contracts work as part of the full request lifecycle.
        </p>
      </div>
      ...
      <section className="mb-16">
        ...
        <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          When a controller handler declares a Contract type hint, Aquilia automatically parses the request body and creates a Contract instance via <code className="text-aquilia-500">bind_contract_to_request()</code>:
        </p>
...
class ProductController(Controller):
    ...
    async def update(self, ctx, id: int, payload: ProductContract):
        ...
        await product.save()
        return ctx.json(ProductContract(instance=product).data)`}</CodeBlock>
      </section>
...
class ProductController(Controller):
    ...
    async def admin_view(self, ctx, id: int):
      ...
      <section className="mb-16">
        ...
        <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Contracts can resolve dependencies from the DI container using the <code className="text-aquilia-500">Inject</code> facet:
        </p>
        <CodeBlock language="python" filename="di_integration.py">{`from aquilia.contracts import Contract, TextFacet, Inject, Hidden
...
class InvoiceContract(Contract):
    ...
    def seal_tax_calculation(self, data):
      ...
      <section className="mb-16">
        ...
        <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Combine Contracts with Aquilia's auth guards for secure endpoints:
        </p>
...
from myapp.contracts import ArticleContract
...
class ArticleController(Controller):
    ...
    async def create(self, ctx, payload: ArticleContract):
        ...
        return ctx.json(ArticleContract(instance=article).data, status=201)
...
# Contract with auth-aware validation:
class AdminSettingsContract(Contract):
    ...
    async def async_seal_admin_only(self, data):
      ...
      <section className="mb-16">
        ...
        <p className={`mb-4 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Contracts can access session data for validation and conditional logic:
        </p>
...
from myapp.contracts import CartContract
...
class CartController(Controller):
    ...
    async def checkout(self, ctx):
        """Checkout flow using session data + Contract."""
        # Get cart items from session
        cart_items = ctx.session.get("cart", [])
        
        # Validate checkout data with Contract
        bp = CheckoutContract(data={
            "items": cart_items,
        ...
        return ctx.json(OrderContract(instance=order).data, status=201)


class CheckoutContract(Contract):
    items = ListFacet(child=IntFacet(), min_items=1)
    shipping_address = NestedContractFacet(AddressContract)
...
class ProductController(Controller):
    ...
    async def retrieve(self, ctx, id: int):
        product = await Product.objects.get(id=id)
        return ctx.json(ProductContract(instance=product, projection="detail").data)
    ...
    @Patch("/{id:int}")
    @guard(AuthGuard)
    async def partial_update(self, ctx, id: int, payload: ProductContract):
        product = await Product.objects.get(id=id)
        # partial=True: only validate provided fields
        bp = ProductContract(data=await ctx.json(), partial=True)
        if not bp.is_sealed():
            return ctx.json(bp.errors, status=422)
    ...
    async def destroy(self, ctx, id: int):
      ...
      <section className="mb-16">
        ...
        <CodeBlock language="python" filename="utilities.py">{`from aquilia.contracts.integration import (
...
# Check if a class is a Contract
is_contract_class(ProductContract)  # True
is_contract_class(str)                # False

# Check if it's a projected Contract reference
is_projected_contract(ProductContract["summary"])  # True

# Resolve Contract from a type annotation
bp_class = resolve_contract_from_annotation(handler_param_annotation)
# Returns (ContractClass, projection_name) or None

# Bind Contract to an incoming request (used internally by controller engine)
bp = await bind_contract_to_request(ProductContract, request, container)

# Render model data through Contract for response
data = render_contract_response(ProductContract, instance, projection="summary")`}</CodeBlock>
      </section>
