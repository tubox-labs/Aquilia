...
export function CacheConfiguration() {
  ...
  const fields: Array<[string, string, string]> = [
    ...
    ['key_builder', 'default | hash', 'Key layout: colon segments or SHA-256 fixed length.'],
    ['key_version', 'int', 'Embedded in every key. Increment to mass-invalidate the keyspace. 0 disables versioning.'],
    ['serializer', 'json | msgpack | pickle', 'Serializer for Redis/composite payloads.'],
    ['serializer_secret_key', 'str', 'HMAC signing key. Required when serializer is "pickle".'],
    ...
    ['distributed_stampede_lock', 'bool', 'Also coalesce across processes when the backend supports locking.'],
    ['stampede_lock_ttl', 'float', 'Lease duration (seconds) for the cross-process lock.'],
    ['stampede_poll_interval', 'float', 'Poll interval (seconds) while awaiting a peer-computed value.'],
    ...
    ['middleware.cache_authenticated', 'bool', 'Nested middleware key. Opt in to caching identity-bearing responses; requires the identity header in vary_headers.'],
