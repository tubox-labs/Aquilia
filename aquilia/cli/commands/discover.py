    ...
    def _run_autodiscovery(self, sync: bool = False, dry_run: bool = False) -> None:
        ...
        for module_name, result in sorted(results.items()):
            for comp in result.components:
                ...
                manifest_refs = engine._parse_manifest_refs(modules_dir / module_name / "manifest.py")
        ...
        if all_new:
            ...
            if sync:
                ...
                dim("  Run `aq discover --sync` to update manifests.")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Inspect auto-discovered modules in workspace")
    parser.add_argument("workspace", help="Workspace name or path")
    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed module information")
    parser.add_argument("--path", help="Workspace path (defaults to workspace name)")
    parser.add_argument("--sync", action="store_true", help="Auto-sync discovered components into manifest.py files")
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be synced without writing (use with --sync)"
    )
