...
class DiscoveryAnalytics:
    ...
    def _calculate_health_score(self, discovered: dict, validation: dict) -> float:
        ...
        modules_with_metadata = sum(
            1 for m in discovered.values() if m.get("author") and m.get("tags") and m["version"] != "0.1.0"
        )
    ...
    def _generate_recommendations(self, discovered: dict, validation: dict) -> list[str]:
        ...
        for name, mod in discovered.items():
            if not mod.get("author"):
                recommendations.append(f"Module '{name}': Add author field to manifest")
            if not mod.get("tags"):
                recommendations.append(f"Module '{name}': Add tags for better discoverability")
            if mod["version"] == "0.1.0":
                recommendations.append(f"Module '{name}': Consider versioning (current: 0.1.0)")

        # Check for dependency issues
        if validation["errors"]:
            recommendations.append(f"Resolve {len(validation['errors'])} dependency errors")

        # Suggest modularity improvements
        if len(discovered) == 1:
            recommendations.append("Consider breaking down functionality into multiple modules")

        # Check for route conflicts
        route_count = len(set(m["route_prefix"] for m in discovered.values()))
        if route_count < len(discovered):
            recommendations.append("Resolve route prefix conflicts (duplicate routes detected)")
