...
class AuditTrail:
    ...
    def __init__(
        self,
        stores: Optional[Sequence[AuditStore]] = None,
    ...
    def _extract_request_info(
        self, request: Any = None, ctx: Any = None,
        ...
        if request is not None:
            ...
            if hasattr(request, "client") and request.client:
                info["ip_address"] = (
                    request.client[0] if isinstance(request.client, tuple)
                    else str(request.client)
                )
    ...
    async def clearance_evaluated(
        ...
        **extra,
        ...
        info = self._extract_request_info(request)
        event_type = (
            AuditEventType.AUTHZ_CLEARANCE_GRANTED if granted
            else AuditEventType.AUTHZ_CLEARANCE_DENIED
        )
        await self.emit(AuditEvent(
