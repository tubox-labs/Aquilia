...
class LifecycleCoordinator:
    ...
    async def startup(self):
        ...
        if self.phase != LifecyclePhase.INIT:
            raise LifecycleError(f"Cannot start from phase {self.phase.value}. Must be in INIT phase.")
        ...
        try:
            ...
            self._emit_event(LifecycleEvent(LifecyclePhase.ERROR, message="Startup failed", error=e))
    ...
    async def _shutdown_app(self, app_name: str):
        ...
        try:
            ...
            self._emit_event(
                LifecycleEvent(
                    LifecyclePhase.STOPPING, app_name=app_name, message=f"{app_name} shutdown error", error=e
                )
            )
