...
def cmd_di_check(args):
    ...
    try:
        ...
        from aquilia.di.errors import DependencyCycleError, DIError, ScopeViolationError
...
def cmd_di_tree(args):
    ...
    try:
        ...
        # Get tree view
        tree = graph.get_tree_view(root=args.root) if args.root else graph.get_tree_view()
            ...
            if args.bench:
                ...
                if registry._providers:
                    ...
                    if median_time * 1_000_000 < 3:
                        print("   Target <3µs: PASSED")
                    else:
                        print("   Target <3µs: FAILED")
...
def cmd_di_manifest(args):
    ...
    try:
        ...
        manifest_data = {
            "version": "1.0",
            "providers": [provider.meta.to_dict() for provider in registry._providers],
            "graph": {token: deps for token, deps in registry._graph.items()},
        }
