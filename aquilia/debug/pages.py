...
def _syntax_highlight_line(code: str) -> str:
    ...
    escaped = re.sub(
        ...
        r'<span style="color:var(--sh-keyword)">\1</span>',
        escaped,
    )
    # Strings
    escaped = re.sub(r"(&quot;.*?&quot;|&#x27;.*?&#x27;)", r'<span style="color:var(--sh-string)">\1</span>', escaped)
    # Comments
...
def _extract_frames(exc: BaseException) -> list[dict[str, Any]]:
    ...
    while tb is not None:
        ...
        locals_dict = (
            {k: v for k, v in frame.f_locals.items() if not (k.startswith("__") and k.endswith("__"))}
            if frame.f_locals
            else {}
        )
...
def _extract_request_info(request: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if hasattr(request, "headers"):
            info["Headers"] = _redact_headers(dict(request.headers))
        if hasattr(request, "query_params"):
            info["Query Params"] = dict(request.query_params)
        if hasattr(request, "path_params"):
            info["Path Params"] = dict(request.path_params)
    except Exception:
        pass
    return info
...
class DebugPageRenderer:
    ...
    def render_http_error(
        status_code: int, message: str = "", detail: str = "", request: Any = None, *, aquilia_version: str = ""
    ) -> str:
...
def render_debug_exception_page(exc: BaseException, request: Any = None, *, aquilia_version: str = "") -> str:
    ...
    if _is_production_environment():
        ...
        return render_http_error_page(
            ...
            request,
            aquilia_version=aquilia_version,
        )
    ...
    for f in reversed(frames):
        ...
        if f["locals"]:
            rows = "".join(
                f'<div class="local-row"><div class="local-key">{_esc(k)}</div><div class="local-val">{_esc(_safe_repr(v))}</div></div>'
    ...
    if req_info:
        for section, data in req_info.items():
            ...
            if isinstance(data, dict):
                rows = "".join(
                    f'<div class="local-row"><div class="local-key">{_esc(k)}</div><div class="local-val">{_esc(str(v))}</div></div>'
                    for k, v in data.items()
                )
                req_html.append(
                    f'<div class="card" style="margin-bottom:16px;"><div class="locals-header">{_esc(section)}</div>{rows}</div>'
                )
            else:
                req_html.append(
                    f'<div class="card" style="margin-bottom:16px;padding:16px;"><strong>{_esc(section)}:</strong> {_esc(str(data))}</div>'
                )
        ...
        <div class="welcome-hero">
            <div>
                ...
                <div style="display:flex;gap:16px;">
                    <a href="https://tubox.cloud" target="_blank" class="btn-primary">
