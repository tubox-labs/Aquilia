...
@pytest.mark.asyncio
async def test_auth_middleware_backends_configuration():
    from aquilia.auth.backends.token import TokenBackend
    from aquilia.auth.core import Identity, IdentityType
    from aquilia.auth.integration.middleware import AquilAuthMiddleware

    session_engine = MagicMock()
    session_engine.resolve = AsyncMock(return_value=None)
    session_engine.commit = AsyncMock(return_value=None)

    fake_identity = Identity(id="user-1", type=IdentityType.USER, attributes={})

    auth_manager = MagicMock()
    auth_manager.token_manager = MagicMock()
    auth_manager.token_manager.validate_access_token = AsyncMock(return_value={"sub": "user-1"})
    auth_manager.verify_token = AsyncMock(return_value={"sub": "user-1"})
    auth_manager.identity_store = MagicMock()
    auth_manager.identity_store.get = AsyncMock(return_value=fake_identity)

    # Token-only backend
    token_backend = TokenBackend(auth_manager.token_manager, auth_manager.identity_store)
    middleware = AquilAuthMiddleware(session_engine=session_engine, auth_manager=auth_manager, backends=[token_backend])
...
@pytest.mark.asyncio
async def test_auth_middleware_backends_configuration_dotted_path():
    from aquilia.auth.core import Identity, IdentityType
    from aquilia.auth.integration.middleware import AquilAuthMiddleware
    ...
    fake_identity = Identity(id="user-2", type=IdentityType.USER, attributes={})

    auth_manager = MagicMock()
    auth_manager.token_manager = MagicMock()
    auth_manager.token_manager.validate_access_token = AsyncMock(return_value={"sub": "user-2"})
    auth_manager.verify_token = AsyncMock(return_value={"sub": "user-2"})
    auth_manager.identity_store = MagicMock()
    auth_manager.identity_store.get = AsyncMock(return_value=fake_identity)

    # Dotted path string for TokenBackend
    middleware = AquilAuthMiddleware(
        session_engine=session_engine, auth_manager=auth_manager, backends=["aquilia.auth.backends.token.TokenBackend"]
    )
