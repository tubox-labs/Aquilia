...
class TestConfigCrossCompatibility:
    ...
    def test_all_configs_have_enabled_in_dict(self):
        from aquilia.db.configs import MysqlConfig, OracleConfig, PostgresConfig, SqliteConfig
...
class TestConfigureDatabaseWithConfig:
    ...
    def test_configure_with_config(self):
        from aquilia.db.configs import SqliteConfig
        from aquilia.db.engine import _database_registry, configure_database
    ...
    def test_configure_default_alias(self):
        ...
        from aquilia.db.engine import _database_registry, configure_database, get_database
...
class TestBigAutoFieldSqlType:
    ...
    def test_mysql(self):
        # BIGINT, not INTEGER: a 32-bit MySQL auto key would cap the table at
        # ~2.1B rows, which is the exact limit BigAutoField exists to lift.
        from aquilia.models.fields_module import BigAutoField

        f = BigAutoField()
        assert f.sql_type("mysql") == "BIGINT"
...
class TestAdminUserMigrationIntegrity:
    ...
    def test_boolean_columns_render_per_dialect(self):
        """The original failure: bool defaults on a BOOLEAN column.

        PostgreSQL rejects ``DEFAULT 1`` on BOOLEAN and ``DEFAULT TRUE`` on
        INTEGER, so the literal has to follow the resolved column type rather
        than the Python value's type.
        """
        from aquilia.models import fields
        from aquilia.models.migration import ColumnState, get_backend

        superuser = ColumnState.of("is_superuser", fields.BooleanField(default=False))
        staff = ColumnState.of("is_staff", fields.BooleanField(default=True))

        postgres = get_backend("postgresql")
        assert postgres.column_definition(superuser) == '"is_superuser" BOOLEAN NOT NULL DEFAULT FALSE'
        assert postgres.column_definition(staff) == '"is_staff" BOOLEAN NOT NULL DEFAULT TRUE'

        # SQLite has no BOOLEAN type: INTEGER with 0/1.
        assert get_backend("sqlite").column_definition(superuser) == '"is_superuser" INTEGER NOT NULL DEFAULT 0'

        # Oracle: NUMBER(1), and DEFAULT must precede NOT NULL (ORA-00907).
        assert get_backend("oracle").column_definition(staff) == '"is_staff" NUMBER(1) DEFAULT 1 NOT NULL'
