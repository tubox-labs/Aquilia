...
class TestProtocolCompliance:
    ...
    def _check_protocol(self, obj):
        ...
        assert isinstance(obj, IntegrationConfig), (
            f"{type(obj).__name__} does not satisfy IntegrationConfig protocol"
        )
...
class TestWorkspaceTypedIntegrate:
    ...
    def test_integrate_cache(self):
        ...
        ws = (
            Workspace("test")
            .integrate(CacheIntegration(backend="redis", max_size=500))
        )
        d = ws.to_dict()
    ...
    def test_integrate_legacy_dict_still_works(self):
        ...
        ws = (
            Workspace("test")
            .integrate(Integration.cache(backend="memory", max_size=2000))
        )
        d = ws.to_dict()
...
class TestEdgeCases:
    ...
    def test_integration_type_preserved_in_dict(self):
        ...
        from aquilia.integrations import (
            AuthIntegration, DatabaseIntegration,
            MailIntegration, CacheIntegration, TasksIntegration,
            StorageIntegration, CorsIntegration, CspIntegration,
            RateLimitIntegration, CsrfIntegration, OpenAPIIntegration,
            I18nIntegration, MLOpsIntegration, VersioningIntegration,
            RenderIntegration, LoggingIntegration, StaticFilesIntegration,
    ...
    def test_workspace_to_dict_roundtrip(self):
        ...
        from aquilia.integrations import (
            CacheIntegration, I18nIntegration, VersioningIntegration,
            CorsIntegration,
        )
        ws = (
